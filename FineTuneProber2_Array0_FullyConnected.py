...
#        rr = targetRows[i]
    ...
    finalADCOut = VADC_read_sec - (secondVRefHiCmp - 3.922) 
    dut.dac_set('DAC_VREF_HI_CMP', 4.0+vOffset)
...
vRead = 0.2
vReadGate = 5.0
Gmap0FineTune = np.zeros((numRows, numCols))
for rr in range(numRows):
        for cc in range(numCols):
            ...
            Gmap0FineTune[rr,cc] = 1e6*rdCurr/vRead
plt.imshow(Gmap0FineTune)
plt.colorbar()

time.sleep(15)
save_workspace(vars(), note='Prober2_FineTuneFCL_ARRAY0')
